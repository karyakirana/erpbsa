<?php namespace App\ERP\Master;

use App\Models\Master\Jabatan;

class JabatanService implements MasterInterface
{
    public function kode()
    {
        $jabatan = new Jabatan();
        return master_kode_helper($jabatan, 'J');
    }

    public function getDataById($id)
    {
        \DB::beginTransaction();
        try {
            $data = Jabatan::find($id);
            return commit_helper($data);
        } catch (\Exception $e){
            return exception_rollback_helper($e);
        }
    }

    public function getData()
    {
        \DB::beginTransaction();
        try {
            $data = Jabatan::all();
            return commit_helper($data);
        } catch (\Exception $e){
            return exception_rollback_helper($e);
        }
    }

    public function getDataIncludeDestroy($active_cash = true)
    {
        // TODO: Implement getDataIncludeDestroy() method.
    }

    public function store(array $data)
    {
        \DB::beginTransaction();
        try {
            $data['kode'] = $this->kode();
            $jabatan = Jabatan::create($data);
            return commit_helper($jabatan);
        } catch (\Exception $e){
            return exception_rollback_helper($e);
        }
    }

    public function update(array $data)
    {
        \DB::beginTransaction();
        try {
            $jabatan = Jabatan::find($data['jabatan_id']);
            $jabatan->update($data);
            return commit_helper($jabatan->refresh());
        } catch (\Exception $e){
            return exception_rollback_helper($e);
        }
    }

    public function softDestroy($id)
    {
        \DB::beginTransaction();
        try {
            $data = Jabatan::destroy($id);
            return commit_helper($data);
        } catch (\Exception $e){
            return exception_rollback_helper($e);
        }
    }

    public function restoreDestroy($id)
    {
        // TODO: Implement restoreDestroy() method.
    }

    public function destroy($id)
    {
        // TODO: Implement destroy() method.
    }
}
